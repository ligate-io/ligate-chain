...
- Operator key file on disk (e.g. `/etc/ligate/keys/themisra-bootstrap.key`), funded with at least 1 AVOW (covers `attestor_set_fee` 0.05 AVOW + `schema_registration_fee` 0.1 AVOW plus generous tx-fee headroom).
...
- **`CannotReserveGas("Insufficient balance")`**: the signer key's address has fewer than ~0.15 AVOW (the ceremony fee total: 0.05 attestor-set + 0.1 schema-reg). Faucet-drip more, retry.
