...
- **Fast per-PR E2E against `local-celestia-devnet`** (closes [#332](https://github.com/ligate-io/ligate-chain/issues/332)). New `.github/workflows/local-celestia-e2e.yml` runs on every PR touching `crates/`, `devnet/`, `localnet/`, `constants.toml`, or `Cargo.{toml,lock}`. Spins up `ghcr.io/evstack/local-celestia-devnet:v0.10.4` as a sidecar service container, boots `ligate-node --da-layer celestia` against it with an ephemeral genesis anchored at the local devnet's current tip, and runs the chain-identity + slot-encoding subset of the mocha-smoke assertions. ~10 min, fully deterministic, no external dependencies. Companion to the nightly real-Mocha smoke ([#285](https://github.com/ligate-io/ligate-chain/issues/285) / [#317](https://github.com/ligate-io/ligate-chain/pull/317)): this catches DA-adapter regressions at PR-review time instead of overnight. Blob-submit/finalize coverage stays in mocha-smoke (this smoke uses a random signer key with no local-celestia funds; a follow-up can swap in the devnet container's known validator key to exercise the full happy path).
...
- **Devnet-2 cutover (chain-id bump + stable-path adoption).** The public devnet rolls from `ligate-devnet-1` to `ligate-devnet-2`, with the AVOW denom and the new `avow_token_id` genesis JSON shape baked in. Mechanics: `devnet-1/` archived to `archive/ligate-devnet-1/` (the genesis files of the dead network kept for historical reference); a fresh `devnet/` created at the role-based stable path adopted in [#483](https://github.com/ligate-io/ligate-chain/pull/483), with `chain_id = "ligate-devnet-2"` in `celestia.toml` and `genesis_da_height` reset to `0` (operator pins to a live Mocha height at boot per `docs/development/runbooks/chain-id-bump.md`). Operator addresses (treasury, sequencer, demo wallets) unchanged from the previous devnet; balances reset to the same 1B AVOW initial distribution (900M / 50M / 50M). Cloud-init paths flip from `/opt/ligate/devnet-1/` to `/opt/ligate/devnet/`. Grafana queries that filter on `chain_id = "ligate-devnet-1"` updated to `ligate-devnet-2`. Closes chain-id ladder bump from rung 1 → 2. ([#493](https://github.com/ligate-io/ligate-chain/pull/493))
- **Devnet-2 prose sweep** (post-cutover doc + code cleanup). Catches the remaining `ligate-devnet-1` references in user-facing prose left over after #493: `constants.toml` docstring, comment examples in `bootstrap-cli` / `client-rs` / `genesis-tool` / `rollup/src/info.rs`, `README.md`, and 15 doc files under `docs/development/` (quickstart, runbooks, public-devnet-deploy guide). Deliberately preserved: anything under `archive/`, the `chain-id-bump.md` runbook (uses devnet-1 as the worked example for future bumps), Grafana dashboard UIDs (provisioning stability), and deployed GCS / Secret Manager names (handled separately by [#496](https://github.com/ligate-io/ligate-chain/pull/496)). Zero behavior change. ([#495](https://github.com/ligate-io/ligate-chain/pull/495))
- **Stable-path naming for deployed infrastructure** (devnet-2 cutover follow-up). Drops the devnet-version suffix from chain-agnostic resources so future devnet-N → devnet-N+1 rollovers stop accumulating renames. Grafana dashboard UID `ligate-devnet-1-cost` → `ligate-cost` (live Grafana renamed via API; repo file `ligate-devnet-1-cost.json` → `ligate-cost.json`). GCS backup bucket `ligate-devnet-1-backups` → `ligate-backups` (new bucket in `us-central1`; `backup.env.example` + `backup-rocksdb.sh` + `ligate-backup@.service` updated; old bucket retained as archive). GCP Secret Manager: `ligate-devnet-1-{celestia-signer-key,grafana-cloud-token}` → `ligate-{celestia-signer-key,grafana-cloud-token}` (chain-agnostic; new secrets created with identical values, VM service account granted accessor; cloud-init flipped). Operator-key backups (`ligate-devnet-1-{operator,attestor,faucet}-key`) intentionally keep the devnet suffix (tied to a specific genesis bank state). ([#496](https://github.com/ligate-io/ligate-chain/pull/496))
- **VM rename follow-ups.** Three small cleanups after the GCE instance rename from `ligate-devnet-1-sequencer` to `ligate-sequencer`: (1) `ops/caddy/Caddyfile` reverse_proxy upstream changed from hardcoded `10.128.0.3:12346` to `127.0.0.1:12346` (Caddy and chain run on the same VM; localhost survives VM ID drift). (2) `ops/cloud-init/sequencer.yaml` GCE create command and `/opt/ligate/devnet-1/` paths updated to the new instance name and stable `/opt/ligate/devnet/`. (3) `ops/grafana/ligate-cost.json` queries filter on `instance="ligate-sequencer"` (live Alloy emits this stable label after `/etc/alloy/config.alloy` was flipped on the running VM). ([#497](https://github.com/ligate-io/ligate-chain/pull/497))
- **Tx latency: drop p50 from ~12s to ~7s via two config tweaks** (`devnet/celestia.toml` + `localnet/celestia.toml`). `batch_execution_time_limit_millis` 12000 → 2000 (sequencer submits a blob to Celestia as soon as 2s have passed since first tx in batch, instead of waiting up to 12s for more batchable txs). `da_polling_interval_ms` 2000 → 500 (chain polls Celestia 4× more frequently for new blobs, cutting follower-side inclusion latency). Trade-off: smaller blobs = slightly more TIA spent per blob at low load; negligible at devnet volume, and going self-DA before mainnet ([#260](https://github.com/ligate-io/ligate-chain/issues/260)) removes the constraint. The hard floor at ~6s per Celestia block remains; further wins need soft confirmations ([#499](https://github.com/ligate-io/ligate-chain/issues/499)) or the native-DA cutover. ([#498](https://github.com/ligate-io/ligate-chain/pull/498))
- **Release builds ~3-4x faster.** Two changes against `release.yml`'s `build:` matrix: (1) `[profile.release]` flips `codegen-units = 1` → `16` in `Cargo.toml`, restoring parallel codegen; (2) `release.yml`'s cache step swaps `actions/cache@v5` → `Swatinem/rust-cache@v2` (the same modern action `ci.yml` already uses), which auto-prunes `target/` to fit the GHA 10GB cache cap, scopes per-toolchain, and pushes warm-cache hit rate from ~50% to ~85%. Trade-off: pre-mainnet binaries lose ~2-5% inter-procedural optimization headroom. Flip `codegen-units` back to `1` (and likely `lto = "fat"`) at the first mainnet RC; release cadence slows then and binary performance starts to matter more than iteration speed. Cold release time on v0.3.0 was ~25-30 min across the matrix; expected ~8-12 min after this change. ([#486](https://github.com/ligate-io/ligate-chain/pull/486))

### Deps

- `actions/checkout` 4 → 6 ([#488](https://github.com/ligate-io/ligate-chain/pull/488)).
- `codecov/codecov-action` 5 → 6 ([#489](https://github.com/ligate-io/ligate-chain/pull/489)).
- `docker/setup-buildx-action` 3 → 4 ([#490](https://github.com/ligate-io/ligate-chain/pull/490)).
- `serde_json` 1.0.149 → 1.0.150 ([#491](https://github.com/ligate-io/ligate-chain/pull/491)).
