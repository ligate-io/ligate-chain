...
/// the canonical SDK config). [`Hash32`] is for content-derived
/// identifiers: schema ids, attestor-set ids, payload hashes, pubkey
/// material. Account fields like [`Schema::owner`] and
/// [`Attestation::submitter`] use `S::Address` directly.
...
pub struct AttestationModule<S: Spec> {
    ...
    #[state]
    pub attestations: StateMap<AttestationKey, Attestation<S>>,
...
impl<S: Spec> AttestationModule<S> {
    ...
    fn handle_submit_attestation(
        ...
        if self.attestations.get(&key, state)?.is_some() {
