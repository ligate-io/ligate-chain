...
/// the attestation module (the canonical chain-defined REST surface)
/// and asserts both the body shape and the correlation header.
///
...
async fn rest_error_envelope_pin() {
    ...
    //
    // `lsc1q...zf30as` is a valid bech32m `lsc1` (32 zero-bytes) — the
    // parser accepts it, the lookup misses, the handler returns
    // `errors::not_found_404("Schema", schema_id)` which the SDK
    // renders as `{status:404, message, details:{id}}`.
    let not_found = client
        .get(format!(
            "{base}/v1/modules/attestation/schemas/lsc1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqzf30as"
        ))
        .send()
        .await
